...
Example usage::
   ...
   times, teams_home, teams_away, spreads = zip(
      *[l.split() for l in pkgdata])
   ...
   nfl_spreads = Melo(k, lines=lines, regress=regress,
      regress_unit=regress_unit, commutes=commutes)

   # fit the estimator to the training data
   nfl_spreads.fit(times, labels1, labels2, spreads, biases=bias)

   # specify a comparison time
   time = nfl_spreads.last_update

   # predict the mean outcome at that time
   mean = nfl_spreads.mean(time, 'CLE', 'KC', biases=bias)
