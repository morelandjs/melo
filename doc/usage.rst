...
The model is calibrated by calling the fit function on the training data. ::
   ...
   melo_instance.fit(times, labels1, labels2, values, bias=0)

Optionally, when training the model you can specify ``bias`` (float or array_like of floats). These are numbers which add to (or subtract from) the rating difference of each comparison, i.e.
...
If ``bias`` is a single number, the bias factor is assumed to be constant for all comparisons.
...
This distribution is generated by the function call ::
   ...
   melo_instance.probability(times, labels1, labels2, bias=bias, lines=lines)

where times, labels1, labels2, and bias are the prediction inputs, and lines is the array of lines where the probability is to be estimated.
...
However, this function call is just the tip of the iceberg. Given this information, the model can predict many other interesting quantities such as the mean and median comparison values ::
   ...
   melo_instance.mean(times, labels1, labels2, bias=bias)

   melo_instance.median(times, labels1, labels2, bias=bias)

...arbitrary percentiles (or quantiles) of the distribution ::

   melo_instance.percentile(times, labels1, labels2, bias=bias, p=[10, 50, 90])

and it can even draw samples from the estimated survival function probability distribution ::

   melo_instance.sample(times, labels1, labels2, bias=bias, size=100)
